...
const handleSavePDF = async () => {
    ...
    if (!input) {
        setIsSubmitting(false);  // 이 경우에도 반드시 풀어줌
        return;
    }
    try {
        // 화면을 캡쳐
        ...
        await showAlert({
            title: "PDF 저장에 실패했습니다.",
            icon: "error"
        });
    } finally {
        setIsSubmitting(false); // 무조건 복구!
    }
};
    ...
    const steps = [
        ...
        { id: 5, title: '완료', icon: FileText }
    ];

    const isFormValid = form.applicantName && form.address && form.phone && form.age && form.signature;
                ...
                <div className="adoption-form-content" ref={pdfRef}>
                    ...
                    {currentStep === 4 && (
                        <div>
                            ...
                            {/* 주거 형태 */}
                            < div className="checklist-item">
                                <div className="checklist-question">주거 형태</div>
                                <div className="checklist-options">
                                    <label>
                                        <input
                                            type="radio"
                            ...
                            {form.houseType === '기타' && (
                                <div style={{ marginTop: '8px' }}>
                                    <input
                                        type="text"
                                        name="houseTypeEtc"
                                        value={form.houseTypeEtc || ''}
                                        onChange={onChange}
                                        placeholder="상세한 주거 형태를 입력해주세요"
                                    />
                                </div>
                            )}
                            ...
                            <div className="checklist-form">
                                <div className="checklist-question">
                                    이사, 또는 해외로 이주 시에 반려동물의 거취 문제에 대해 어떻게 생각하십니까?
                                </div>
                                <div className="conditional-section">
                                    <textarea
                                        name="movingPlan"
                                        value={form.movingPlan}
                                        onChange={onChange}
                                        rows={3}
                                        placeholder="구체적인 계획을 설명해주세요"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="checklist-form">
                                <div className="checklist-question">
                                    미혼의 경우 결혼·임신·출산 시 입양하신 반려동물의 거취에 대해 어떻게 생각하십니까?
                                </div>
                                <div className="conditional-section">
                                    <textarea
                                        name="futurePlan"
