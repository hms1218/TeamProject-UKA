...
const ChatList = () => {
    ...
    const ChatPosts = sortPosts(posts.filter(p => p.category === "CHAT"));
    ...
    const handleSearch = () => {
        ...
        const filtered = ChatPosts.filter(post => {
    ...
    const handleTitleClick = (post) => {
        navigate(`/board/chat/detail/${post.id}`, {
            state: {
                filteredList: isSearching ? filteredPosts : displayedPosts,
